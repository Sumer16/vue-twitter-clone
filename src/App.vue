<template>
  <div class="flex flex-col min-h-screen">
    ...
    <footer class="grid grid-cols-4 border-t border-gray-600">
      <router-link v-for="route in routes" :key="route.name" :to="route.path" :class="`p-4 text-center text-2xl text-gray-300 ${(route.name == $route.name) ? 'text-green-500' : 'text-gray-300'}`">
